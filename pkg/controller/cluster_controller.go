...
func (r *ClusterReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	...
	tracelog.EmitComparable(ctx, log, "CockroachOperatorSpecObserved", nil, map[string]any{
		"specNodes":  cluster.Spec().Nodes,
		"tlsEnabled": cluster.Spec().TLSEnabled,
	}, &tracelog.NondeterministicHints{K8s: []string{"specNodes", "tlsEnabled"}})
	tracelog.EmitComparable(ctx, log, "CockroachOperatorStatusObserved", nil, map[string]any{
	...
	tracelog.EmitComparable(ctx, log, "ReconcileStart", map[string]any{
